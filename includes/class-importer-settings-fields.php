<?php
/**
 * Handles the registration and rendering of settings fields.
 */
class Polarsteps_Importer_Settings_Fields {
    ...
    public function post_category_render() {
        ...
        jQuery(document).ready(function($) {
            $('#polarsteps_post_type').change(function() {
                ...
                // Simple reload of page might be easier to trigger category refresh in a real scenario,
                // but here we just try to fetch categories dynamically if possible, or show a message.
                // Since data-taxonomies isn't easily available here without more complex logic, 
                // we will rely on the backend saving the post type and the user refreshing or saving to see new categories correctly if they switch post types.
                // But for now, let's keep the existing UI logic structure.
                
                // Logic to update categories via AJAX would go here.
            });
        });
        </script>
        <?php
    ...
    public function sanitize_settings($input) {
        ...
        if (!$next_scheduled && !empty($sanitized_input['polarsteps_trip_id']) && !empty($sanitized_input['polarsteps_remember_token'])) {
            ...
            Polarsteps_Importer_Settings::log_message(__('Trip ID and Token set. Recurring cron job scheduled.', 'polarsteps-importer'));
            ...
            Polarsteps_Importer_Cron::schedule_recurring_event();
            Polarsteps_Importer_Settings::log_message(__('Update interval changed. Cron job has been rescheduled.', 'polarsteps-importer'));
        }
